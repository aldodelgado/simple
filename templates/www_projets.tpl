				...
				<table class="table table-striped">
					<tr style="font-weight:bold;">
						<td colspan="2" align="center">
							{if $order eq "nom"}
								{if $by eq "asc"}
									<a href="?order=nom&by=desc">{#projet_liste_projet#} ({$projets|@count})</a>&nbsp;<img src="{$BASE}/assets/img/pictos/asc_order.png" border="0" alt="" />
								{else}
									<a href="?order=nom&by=asc">{#projet_liste_projet#} ({$projets|@count})</a>&nbsp;<img src="{$BASE}/assets/img/pictos/desc_order.png" border="0" alt="" />
								{/if}
							{else}
					...
					{foreach from=$projets item=projet}
						...
						<tr style="background-color:{$couleurLigne};" onMouseOver="javascript:this.style.backgroundColor='#EEEEEE'" onMouseOut="javascript:this.style.backgroundColor='{$couleurLigne}'">
							<td width="25" style="font-size:12px;background-color:#{$projet.couleur};color:{"#"|cat:$projet.couleur|buttonFontColor}">{$projet.projet_id}</td>
							<td>
								{if in_array("projects_manage_all", $user.tabDroits) || (in_array("projects_manage_own", $user.tabDroits) && $projet.createur_id eq $user.user_id)}
									<a onClick="javascript:xajax_modifProjet('{$projet.projet_id}', 'projets');undefined;" style="cursor:pointer;"><img src="{$BASE}/assets/img/pictos/edit.gif" border="0" width="16" height="16" align="absbottom" /></a>
									&nbsp;
									<a  href="javascript:xajax_supprimerProjet('{$projet.projet_id|urlencode}');undefined;" onClick="javascript:return confirm('{#projet_liste_confirmSuppr#|escape:"javascript"}')"><img src="{$BASE}/assets/img/pictos/delete.gif" border="0" width="16" height="16" align="absbottom" /></a>
								{/if}
								&nbsp;
								<a href="{$BASE}/process/planning.php?filtreSurProjet={$projet.projet_id}" title="{#planning_filtre_sur_projet#|escape}" alt="{#planning_filtre_sur_projet#|escape}"><img src="{$BASE}/assets/img/pictos/logo.png" border="0" width="18" height="18" align="absbottom" /></a>
								&nbsp;

...
<form class="form-horizontal" method="POST" action="" target="_blank">
	...
	<div class="control-group">
		...
		<div class="controls">
			{if in_array("projects_manage_all", $user.tabDroits)}
				<select name="createur_id" id="createur_id">
	...
	<div class="control-group">
		...
		<div class="controls">
			<input name="iteration" id="iteration" size="30" maxlength="255" type="text" value="{$projet.iteration}" /> {#winProjet_commentairesFacultatif#}
		</div>
	</div>
	<input type="button" value="{#winProjet_valider#|escape:"html"}" class="btn btn-primary" style="margin-left: 180px;" onClick="xajax_submitFormProjet('{$projet.projet_id}', $(origine).value, $(projet_id).value, $(nom).value, $(groupe_id).value, $(statut).value, $(charge).value, $(livraison).value, $(couleur).value, $(createur_id).value, $(iteration).value)" />
</form>
	...
	end : function(e)
	{
		...
		if(origineCaseX != parseInt(Drag.obj.style.left) || origineCaseY != parseInt(Drag.obj.style.top)) {
			...
			if (destination == null) {
				...
				document.getElementById('divChoixDragNDrop').style.top = getPosition(Drag.obj, 'offsetTop') + "px";
...
function initselect2() {
	...
	jQuery(".select2").each(function() {
		jQuery('#' + this.name).select2({allowClear: true});
	});
